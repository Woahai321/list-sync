...
ARG NUXT_PUBLIC_API_URL=${NUXT_PUBLIC_API_URL:-http://localhost:4222}
ENV NUXT_PUBLIC_API_URL=${NUXT_PUBLIC_API_URL:-http://localhost:4222}

# Copy package files
COPY listsync-nuxt/package*.json ./
...
COPY listsync-nuxt/pages ./pages
COPY listsync-nuxt/components ./components
COPY listsync-nuxt/composables ./composables
COPY listsync-nuxt/stores ./stores
COPY listsync-nuxt/services ./services
COPY listsync-nuxt/types ./types
COPY listsync-nuxt/utils ./utils
COPY listsync-nuxt/layouts ./layouts
COPY listsync-nuxt/assets ./assets
COPY listsync-nuxt/public ./public
COPY listsync-nuxt/nuxt.config.ts ./
COPY listsync-nuxt/tailwind.config.js ./
COPY listsync-nuxt/tsconfig.json ./
COPY listsync-nuxt/app.vue ./
COPY listsync-nuxt/error.vue ./

# Ensure proper permissions for Node.js binaries
RUN chmod +x node_modules/.bin/*

# Build the frontend (tsconfig files generated by build hook in nuxt.config.ts)
RUN npm run build
...
# Copy built Nuxt 3 application from builder
COPY --from=node-builder /app/frontend/.nuxt ./frontend/.nuxt
COPY --from=node-builder /app/frontend/.output ./frontend/.output
...
COPY listsync-nuxt/nuxt.config.ts ./frontend/
COPY listsync-nuxt/tailwind.config.js ./frontend/
...
RUN echo "[supervisord]" > /etc/supervisor/conf.d/listsync.conf && \
    ...
    echo "command=node .output/server/index.mjs" >> /etc/supervisor/conf.d/listsync.conf && \
    echo "directory=/usr/src/app/frontend" >> /etc/supervisor/conf.d/listsync.conf && \
    echo "environment=PORT=3222,HOST=0.0.0.0" >> /etc/supervisor/conf.d/listsync.conf && \
    echo "autostart=true" >> /etc/supervisor/conf.d/listsync.conf && \
    echo "autorestart=true" >> /etc/supervisor/conf.d/listsync.conf && \
