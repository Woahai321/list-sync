...
def fetch_trakt_list(list_id: str) -> List[Dict[str, Any]]:
    ...
    try:
            ...
            while True:
                ...
                for item in items:
                    try:
                        ...
                        logging.warning(f"Failed to parse Trakt item: {e!s}")
                        continue
        ...
        logging.exception(f"Error fetching Trakt list {list_id}: {e!s}")
        raise
...
def fetch_trakt_special_list(url_or_shortcut: str) -> List[Dict[str, Any]]:
    ...
    try:
        with SB(uc=True, headless=True) as sb:
            ...
            for item in items:
                ...
                try:
                    ...
                    logging.warning(f"Failed to parse Trakt special list item: {e!s}")
                    continue
        ...
        logging.exception(f"Error fetching special Trakt list: {e!s}")
        raise
