...
def fetch_imdb_list(list_id):
    ...
    try:
        ...
        movies = [
            {
                "title": html.unescape(row["item"]["name"]),
                "imdb_id": row["item"]["url"].split("/")[-2],
            }
            for row in ld_json["itemListElement"]
        ]
        spinner.succeed(
            color_gradient(
                f"✨  Successfully fetched {len(movies)} movies from IMDB list!",
                "#00ff00",
                "#00aa00",
            )
        )
        ...
        spinner.fail(
            color_gradient(
                f"💥  Failed to fetch IMDB list. Error: {str(e)}", "#ff0000", "#aa0000"
            )
        )
...
def search_movie_in_overseerr(overseerr_url, api_key, movie_title):
    ...
    try:
        ...
        logging.debug(
            f'Search response for "{movie_title}": {json.dumps(search_results)}'
        )
...
def confirm_movie_status(overseerr_url, api_key, movie_id):
    ...
    try:
        ...
        logging.debug(
            f"Detailed response for movie ID {movie_id}: {json.dumps(movie_data)}"
        )
        ...
        logging.debug(
            f"Movie ID {movie_id} - Available: {is_available_to_watch}, Requested: {is_requested}"
        )
...
def request_movie_in_overseerr(overseerr_url, api_key, movie_id, media_type):
    ...
    try:
        ...
        response.raise_for_status()
        logging.debug(
            f"Request response for movie ID {movie_id}: {json.dumps(response.json())}"
        )
        return "success"
    except Exception as e:
...
def main():
    ...
    while True:
        ...
        for idx, movie in enumerate(movies, 1):
            ...
            try:
                search_result = search_movie_in_overseerr(
                    overseerr_url, api_key, movie["title"]
                )
                if search_result:
                    # Confirm movie status before requesting
                    is_available_to_watch, is_requested = confirm_movie_status(
                        overseerr_url, api_key, search_result["id"]
                    )
                    if is_available_to_watch:
                        ...
                        request_status = request_movie_in_overseerr(
                            overseerr_url,
                            api_key,
                            search_result["id"],
                            search_result["mediaType"],
                        )
